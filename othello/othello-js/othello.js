...
(function () {
  ...
  var m = location.href.match(/\?n=(\d+)$/);
    // N = m. if m is null, it equals 8. if not, gets the first move
  ...
  var BLACK = 'black';
    ...
    // takes coordinates multiplies by N
  function ix(x, y) {
    return x + y * N;
  }
  ...
  function makeInitialGameBoard() {
    ...
    for (var x = 0; x < N; x++)
      for (var y = 0; y < N; y++)
        board[ix(x, y)] = EMPTY;
      // what is this
      // the bits of N shift one to the right and it is save in var
    var x2 = N >> 1;
    var y2 = N >> 1;
    // puts the first four pieces on board
  ...
  function completePassingMove(attackingMoves, board, player, wasPassed, nest) {
    // if there are possible moves, return them
    if (0 < attackingMoves.length)
      return attackingMoves;
    else if (!wasPassed)
    // if have not passed yet return there is a passing move. What is nest
      return [{
        isPassingMove: true,
        // is delay defined elsewhere
        // wasPassingMoveIsTrue
        // what is nest
  ...
  function judge(board) {
    // n is number?
    // judge is called when game is over
    var n = {};
    n[BLACK] = 0;
    n[WHITE] = 0;
    n[EMPTY] = 0;
...
function makeRandomAi() {
  ...
  */
  ...
  // O equals othello so it inherits from these
